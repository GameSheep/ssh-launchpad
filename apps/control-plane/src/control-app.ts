...
export interface ControlAppDependencies extends ControlRouteDependencies {
...
function hostAllowed(value: string | undefined, publicBaseUrl: string): boolean {
  ...
  let host: URL
  try { host = new URL(publicBaseUrl) } catch { return false }
  const incoming = value.split(':')[0]?.toLowerCase()
  return incoming === host.hostname.toLowerCase() || incoming === 'localhost' || incoming === '127.0.0.1' || incoming === '[::1]'
}
...
export async function buildControlApp(dependencies: ControlAppDependencies): Promise<FastifyInstance> {
  ...
  app.get('/agent', { websocket: true }, (socket) => dependencies.gateway.handle(socket))
  if (dependencies.webRoot) {
    await app.register(fastifyStatic, { root: dependencies.webRoot, prefix: '/', decorateReply: false })
    app.setNotFoundHandler((request, reply) => {
      if (request.method === 'GET' && !request.url.startsWith('/api/') && !request.url.startsWith('/agent') && request.headers.accept?.includes('text/html')) return reply.sendFile('index.html')
