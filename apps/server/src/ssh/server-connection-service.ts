...
export interface ServerConnectionService {
  test(serverId: string, credential?: string): Promise<{ ok: true } | { ok: false; candidateFingerprint: string }>
...
export class DefaultServerConnectionService implements ServerConnectionService {
  ...
  async test(serverId: string, credential?: string): Promise<{ ok: true } | { ok: false; candidateFingerprint: string }> {
    ...
    try {
      const session = await this.factory.connect(server, credential ?? secret)
      await session.close()
