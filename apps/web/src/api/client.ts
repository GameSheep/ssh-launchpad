...
export const api = {
  ...
  connect: (id: string, serverId: string) => request<{ url: string; status: 'healthy' }>(`/api/apps/${id}/connect`, { method: 'POST', body: JSON.stringify({ credential: browserCredentials.get(serverId) }) }),
  disconnect: (id: string) => request<{ ok: true }>(`/api/apps/${id}/disconnect`, { method: 'POST', body: '{}' }),
  reconnect: (id: string, serverId: string) => request<{ url: string; status: 'healthy' }>(`/api/apps/${id}/reconnect`, { method: 'POST', body: JSON.stringify({ credential: browserCredentials.get(serverId) }) }),
