...
export function ControlLogin() {
  ...
  return <main className="control-shell"><section className="control-card"><div className="brand-mark">⌁</div><span className="eyebrow">CONTROL PLANE</span><h1>登录 SSH <i>Launchpad</i></h1><p>输入部署时设置的 CONTROL_TOKEN。SSH 密码只保存在当前浏览器，不会保存到服务端。</p><form onSubmit={submit}><label>控制令牌<input autoFocus required type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="CONTROL_TOKEN" /></label>{message && <div className="control-message">{message}</div>}<button className="primary" disabled={busy}>{busy ? '验证中…' : '进入工作台'}</button></form></section></main>
}
