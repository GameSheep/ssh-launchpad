...
import type { ServerInput } from '@ssh-launchpad/shared'
import { api } from '../api/client.js'

const initial: ServerInput = { name: '', source: 'manual', host: '', port: 22, username: '', authType: 'password', notes: '' }

export function ServerDialog({ onClose, onSave }: { onClose(): void; onSave(input: ServerInput, credential?: { kind: 'password' | 'private-key-passphrase'; value: string }): Promise<void> }) {
  const [value, setValue] = useState<ServerInput>(initial)
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <form className="modal" onSubmit={submit}>
      <header className="modal-header"><div><span className="eyebrow">配置入口</span><h2>添加服务器</h2></div><button type="button" className="modal-close" onClick={onClose}>×</button></header>
      <div className="modal-body">
        ...
        <div className="form-grid">
          ...
          <label>凭据 {secret ? <small>已填写</small> : null}<input type="password" value={secret} onChange={(event) => setSecret(event.target.value)} placeholder="只保存到当前浏览器" /></label>
      ...
      <footer className="modal-footer"><span className="security-note">⌁ 凭据只保存在当前浏览器，不会写入服务端</span><button type="button" className="ghost" onClick={onClose}>取消</button><button className="primary" disabled={saving}>{saving ? '保存中…' : '保存服务器'}</button></footer>
    </form>
  </div>
}
